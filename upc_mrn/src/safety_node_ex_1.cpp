...
public:
  ...
  void cmdvelCallback(const geometry_msgs::msg::Twist &msg)
  {
    ...
    if (abs(linear_velocity) > v_max_) { //check if the current velocity is bigger than the stablished max 
      RCLCPP_INFO(get_logger(), "Implementing SafetyNode! current velocity %f > max linear velocity = %f",linear_velocity,v_max_);

      linear_velocity =  v_max_* (linear_velocity/abs(linear_velocity)); // reduce by a ratio the linear velocity 
